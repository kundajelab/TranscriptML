...
from transcriptml.analysis.codon_tables import (
    bin_columns as shared_bin_columns,
    position_bin as shared_position_bin,
    safe_filename as shared_safe_filename,
)
...
def default_data_dir() -> Path:
    """Return a conventional local training-data directory fallback."""

    relative = Path("Training") / "data"
    if relative.exists():
        return relative
...
def default_out_dir(data_dir: Path) -> Path:
    ...
    for parent in data_dir.resolve().parents:
        ...
        if candidate.exists():
            return candidate / "codon_usage"
    return data_dir.parent / "codon_usage"
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "fractions from a TranscriptML Saluki-style dataset bundle."
        ),
...
def position_bin(codon_index: int, n_codons: int, n_bins: int) -> int:
    return shared_position_bin(codon_index, n_codons, n_bins)
...
def bin_columns(bin_id: int, n_bins: int) -> dict[str, float | int]:
    return shared_bin_columns(bin_id, n_bins)
...
def safe_filename(value: str) -> str:
    return shared_safe_filename(value)
...
def run_codon_usage_from_args(args: argparse.Namespace) -> int:
    """Run codon usage analysis from parsed CLI args."""
    ...
    global_counts, bin_counts, summary = analyze_usage(
        ...
        cds_index=cds_index,
        n_bins=args.n_bins,
        limit_transcripts=getattr(args, "limit_transcripts", None),
    )
    summary.update(
        {
...
def main(argv: Iterable[str] | None = None) -> int:
    return run_codon_usage_from_args(parse_args(argv))
