...
- `rbpnet/`: **experimental** eCLIP preprocessing, scan/selection/bundle construction, immutable balanced chromosome CV planning, RBPNet training, and structured scientific evaluation per chromosome fold. See `rbpnet/README.md`.

> **Experimental RBPNet status:** Every RBPNet/eCLIP stage is minimally tested.
> The workflow has only been confirmed to preprocess data successfully and train
> reasonable models on PUM2 eCLIP data, and it needs substantially more
